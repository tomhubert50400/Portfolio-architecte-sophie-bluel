...
async function deleteWork(workId) {
  ...
  if (!confirm("Êtes-vous sûr de vouloir supprimer ce projet ?")) {
    // On demande une confirmation avant de supprimer le projet
    return; // Si l'utilisateur annule, on ne fait rien
  }

  try {
    const response = await fetch(`http://localhost:5678/api/works/${workId}`, {
      // On envoie une requête DELETE à l'API pour supprimer le projet
    ...
    if (response.ok) {
      // Si la requête a réussi je met à jour la modale en avertissant l'utilisateur
      alert("Projet supprimé avec succès !");
      displayWorksInModal();
    } else {
      // Si la requête a échoué j'affiche un message d'erreur à l'utilisateur
...
document
  ...
  .addEventListener("submit", async (event) => {
    ...
    try {
      const response = await fetch("http://localhost:5678/api/works", {
        // On envoie une requête POST à l'API pour ajouter un projet
        method: "POST",
        headers: {
          Authorization: `Bearer ${localStorage.getItem("token")}`, // On envoie le token dans le header de la requête
        },
